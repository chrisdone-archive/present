...
data Presentation
  ...
  | StringPresentation String
                       String
  | TuplePresentation String [Presentation]
  deriving (Show,Eq)

-- | Make a presenter for a type definition.
typeDefinitionPresenter :: TypeDefinition -> Q [Dec]
typeDefinitionPresenter =
  \case
    DataTypeDefinition typeConstructor@(TypeConstructor typeConstructorName) dataType@(DataType typeVariables _) ->
      do case lookup typeConstructorName builtInPresenters of
           Nothing -> dataTypePresenter typeConstructor dataType
           Just presenter ->
             do automaticPresenter <-
                  dataTypePresenterBody typeConstructor dataType
                builtInPresenter typeConstructor (presenter typeVariables automaticPresenter)
    TypeAliasDefinition typeConstructor typeAlias ->
      typeAliasPresenter typeConstructor typeAlias

-- | Make a built-in printer for a data type.
builtInPresenter :: TypeConstructor -> Q Exp -> Q [Dec]
builtInPresenter typeConstructor presenter =
  builtinFunctionDeclaration typeConstructor presenter

-- | Make a printer for a data type.
  ...
  where typeDisplayExpression = typeDisplay typeVariables typeConstructorName
        constructorCase (Constructor (ValueConstructor valueConstructorName) fields) =
          match (conP valueConstructorName (map (return . fieldPattern) indexedFields))
                (normalB (appE (appE (appE (conE 'DataTypePresentation)
          ...
          where indexedFields = zip (map indexedFieldName [0 ..]) fields
                ...
                fieldPresenter (indexedName,(_mvalueVariable,normalType)) =
                  appE (appE (varE 'snd)
...
builtInPresenters :: [(Name,[TypeVariable] -> Exp -> Q Exp)]
builtInPresenters = concat [listPrinters,integerPrinters,charPrinters]
  where listPrinters =
          [(''[]
                             ...
                             ,\xs ->
                                case fst $(presentVar) of
                                  "GHC.Types.Char" ->
                                    ...
                                    where getCh (CharPresentation "GHC.Types.Char" ch) = ch
...
makeTypePresenter getTy =
  do ty <- getTy
     ...
     case normalizeResult of
       Left err -> fail err
       Right normalType ->
         do typeDefinitions <- normalTypeDefinitions normalType
            presenters <- mapM typeDefinitionPresenter typeDefinitions
...
toShow qualified =
  \case
    ...
    ChoicePresentation _ ((_,x):_) -> toShow qualified x -- Pick the first choice.
    ChoicePresentation _ [] -> ""
  where recur p
          ...
          where atomic =
                  \case
                    ...
                    ChoicePresentation _ ((_,x):_) -> atomic x
